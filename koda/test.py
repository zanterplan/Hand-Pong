...
# Symbol images
plus_image = pygame.image.load("green.png").convert_alpha()
plus_image = pygame.transform.scale(plus_image, (ball_radius * 2, ball_radius * 2))

minus_image = pygame.image.load("red.png").convert_alpha()
minus_image = pygame.transform.scale(minus_image, (ball_radius * 2, ball_radius * 2))

infinity_image = pygame.image.load("infinity.png").convert_alpha()
infinity_image = pygame.transform.scale(infinity_image, (ball_radius * 2, ball_radius * 2))

question_image = pygame.image.load("question.png").convert_alpha()
question_image = pygame.transform.scale(question_image, (ball_radius * 2, ball_radius * 2))

# Background image
park_bg = pygame.image.load("park.jpg").convert()
night_bg = pygame.image.load("night.jpg").convert()
blurred_bg = pygame.transform.scale(park_bg, (WIDTH, HEIGHT))

# Load sound effect
bounce_sound = pygame.mixer.Sound("ball.wav")
collect_plus = pygame.mixer.Sound("collect_plus.wav")
collect_minus = pygame.mixer.Sound("collect_minus.wav")
collect_ball = pygame.mixer.Sound("collect_ball.wav")
collect_powerup = pygame.mixer.Sound("collect_powerup.wav")
...
wind_active = False
wind_start_time = 0
next_wind_time = pygame.time.get_ticks() + random.randint(2000, 3000)
wind_duration = 2500
wind_vx = 0
wind_vy = -1
...
def draw_text_and_image(txt, img, height):
    # Font
    font = pygame.font.Font("joystix.otf", 28)
...
def draw_title(title):
    title_font = pygame.font.Font("joystix.otf", 54)
...
def spawn_object():
    ...
    bonus_objects.append({"x": x, "y": y, "type": object_type, "radius": 20, "duration": 6000, "object_spawn_time": pygame.time.get_ticks()})
...
def spawn_additional_ball(ball_x, ball_y):
    global balls
    balls.append({"x": ball_x, "y": ball_y, "vx": random.choice([-5, 5]), "vy": random.choice([-10, -5])})
...
while running:
    ...
    if game_paused:
        ...
        pause_font = pygame.font.Font("joystix.otf", 54)
    ...
    if not game_over and not game_paused and not countdown_active and game_started:
        ...
        if not wind_active and current_time >= next_wind_time:
            ...
            next_wind_time = current_time + random.randint(8000, 10000)
            wind_vx = random.choice([-1, 1])
            wind_vy = -1
